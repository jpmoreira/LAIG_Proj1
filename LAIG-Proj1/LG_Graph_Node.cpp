...
#define LG_Graph_Node_Att_Visible "visible"
#define LG_Graph_Node_Att_Selectable "selectable"
	...
	:LG_Parsable_Node(map, identifierForGraphNode(elem)), animations(vector<LG_AnimationState *>()), appearance(NULL), currentAnimation(0)
{
	...
	try {
		bool_tryToAttributeVariable(LG_Graph_Node_Att_Selectable, elem, selectable);
	}
	...
	try {
		bool_tryToAttributeVariable(LG_Graph_Node_Att_Visible, elem, visible);
	}
	catch (LG_Parse_Exception *ex)
	{
		visible = true;
...
void LG_Graph_Node::draw(){
	...
	if (isDisplayList){
		...
		for (unsigned int i = 0; i < childsIDs.size(); i++) {
			...
			try{
				LG_Primitive * type = dynamic_cast<LG_Primitive *> (child(i));
