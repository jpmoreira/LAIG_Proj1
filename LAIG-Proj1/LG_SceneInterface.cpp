...
void LG_SceneInterface::initGUI()
{
	...
	//--------------------------------------------------------------------------	
	glui_window->add_column(false);
	GLUI_Panel *wind_pan = addPanel("Wind Panel", 1);
	GLUI_Spinner *wind_spinner = addSpinnerToPanel(wind_pan, "Wind", 2, wind_var, -1);
	wind_spinner->set_float_limits(0, 10, 1);
	addSeparator();

	/*afloat = 0;
	GLUI_Spinner *spinner = new GLUI_Spinner(wind_pan, "Spinner", &afloat, -1);
	spinner->set_float_limits(-10, 10, 1);
...
void LG_SceneInterface::setWind_var(int * flag_windvar)
{
	this->wind_var = flag_windvar;
}